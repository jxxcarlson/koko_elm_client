...
prepareExportLatexFromMaster model =
    let
        ...
        maybeAuthor = KeyValue.getStringValueForKeyFromTagList "author" model.current_document.tags  
        maybeDate = KeyValue.getStringValueForKeyFromTagList "date" model.current_document.tags  

        titleCommand = "\\title{" ++ model.current_document.title ++  "}"

        authorCommand = case maybeAuthor of
          Just author ->  "\\author{" ++ author ++ "}"
          Nothing -> ""

        dateCommand = case maybeDate of
          Just date -> "\\date{" ++ date ++ "}"
          Nothing -> ""

        commands = [titleCommand, authorCommand, dateCommand, "\\maketitle", "\\tableofcontents"] |> String.join("\n")
