module LatexParser.TextSample exposing (..)

{-| EXAMPLES:

    qftIntroText1 -- root example

    qftIntroText2 -- Change "You may say, OK, all that philosophy is fine" to
    "You may say, Wow! all that philosophy is fine"

-}


qftIntroText1 : String
qftIntroText1 =
    """
...
You may say, Wow! all that philosophy is fine, but let's rig thing so that $\\Delta x$ is so small that we might as well consider out particle to be a classical point particle.  But if $\\Delta x$ is very, very small, then the uncertainty principle ensures that $\\Delta p$ will be very, very large, as will be $\\Delta v = \\Delta p/m$.  In our attempt to recover a classical trajectory, we make $\\Delta p$ so large that the notion of a well defined velocity is meaningless. Thus, whatever the mathematical language of quantum mechanics is, it cannot be based on a function ${\\bf r}(t)$ which gives position and therefore also velocity ${\\bf r}(t)$ and momentum ${\\bf p}(t)$.
